...
export const onPushQueueCreated = functions
  .region('us-central1')
  .firestore
  .document('push_queue/{notifId}')
  .onCreate(async (snap, context) => {
    const notifId = context.params.notifId;
    const data = snap.data() as PushQueueItem;
    ...
    try {
      ...
      if (invalidTokens.length > 0) {
        ...
        const deletePromises = tokensSnap.docs
          .filter(d => invalidTokens.includes((d.data() as FcmToken).token))
          .map(d => d.ref.delete());
...
export const sendPendingNotifications = functions
  .region('us-central1')
  .pubsub
  ...
  .onRun(async () => {
    ...
    const tokens = tokensSnap.docs
      .map(d => (d.data() as FcmToken).token)
      .filter(t => t && t.length > 20);

    if (tokens.length === 0) return null;

    for (const doc of pending.docs) {
      const data = doc.data() as PushQueueItem;
      try {
...
export const weeklyCleanup = functions
  ...
  .onRun(async () => {
    ...
    if (!oldNotifs.empty) await batch.commit();
...
export const weeklyAdminSummary = functions
  .region('us-central1')
  .pubsub
    ...
    try {
      ...
      if (!usersSnap.empty) {
        ...
        if (adminTokenDoc.exists) {
          const token = (adminTokenDoc.data() as FcmToken).token;
